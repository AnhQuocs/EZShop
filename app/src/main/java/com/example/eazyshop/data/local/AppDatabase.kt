...
import androidx.room.RoomDatabase.Callback // Import Callback
...
@Database(entities = [Product::class, CartItem::class, Address::class, OrderHistory::class, User::class], version = 20, exportSchema = false)
        ...
        fun getDatabase(context: Context): AppDatabase {
            return INSTANCE ?: synchronized(this) {
                val instance = Room.databaseBuilder(
                    ...
                    .addCallback(RoomCallback(context)) // Sử dụng RoomCallback
                    .build()
                INSTANCE = instance
                instance
            }
...
private class RoomCallback(private val context: Context) : Callback() {
    override fun onCreate(db: SupportSQLiteDatabase) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            // Kiểm tra xem bảng user đã có dữ liệu chưa
            val userDao = database.userDao()
            if (userDao.getUser(1) == null) {
                populateDatabase(database.productDao(), userDao)
            }
        }
    }
}
